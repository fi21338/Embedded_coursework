...
// TODO: Function to implement FIR filtering
float filter1(float Input_Data)
{
    int i;
    float sum = 0;

    // shift old samples from filtbuff1 to make space for the new input sample
    for (i = FIR_FILTER_LENGTH - 1; i > 0; i--) {
        filtbuff1[i] = filtbuff1[i - 1];
    }

    // add the new sample at the start of the buffer
    filtbuff1[0] = Input_Data;

    // convolution of the input with the FIR coefficients
    for (i = 0; i < FIR_FILTER_LENGTH; i++) {
        // weighted sum of the samples
        sum += filtbuff1[i] * firCoeffs[i];
    }
...
Explanation of FIR Filter Theory (3 marks):


The FIR filter removes unwanted parts of a signal by calculating a weighted sum of the current and past inputs (convolution).
The weights, called coefficients (firCoeffs[]), control which parts of the signal are kept or reduced.

The filtering is done in the filter1() function. Here, each new input sample is added to a buffer (filtbuff1[]),
shifting older samples down. The function then multiplies each sample in the buffer by its weight and sums them up.
This sum is the filtered output. Organizing this in filter1() makes the filtering process clear, efficient and centralized.
